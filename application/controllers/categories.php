...
class Categories extends MY_Controller {
	...
	function browse($loc_en=NULL, $topic_en=NULL)
	{
		$where = array();
		if($loc_en) {
			$where["location_categories.loc_name"] = ($loc_en) ? rawurldecode($loc_en) : "";
		}
		if($topic_en) {
			$where["topics_categories.topic_name"] = ($topic_en) ? rawurldecode($topic_en) : "";
		}
		$submissions = $this->Categories_model->get_submissions($where);
		$title = (($loc_en) ? rawurldecode($loc_en)." > " : "") . (($topic_en) ? rawurldecode($topic_en) : "");
		if($title == ""){
