...
class Users_model extends CI_Model {
	...
	function get_account_level($username = "") {
	...
	function promote_to_volunteer ($username) {
		mysql_query("UPDATE `users` SET  `level` =  '1' WHERE  `users`.`username` =  '$username'");
		
	}
	function demote_from_volunteer ($username) {
		mysql_query("UPDATE `users` SET  `level` =  '0' WHERE  `users`.`username` =  '$username'");
    ...
    function retScore ($uid) {
    	$result = mysql_query("SELECT SUM(Score) AS A FROM submissions WHERE username='$uid' and Score IS NOT NULL");
    	echo mysql_error();
    	if (!$result) {
    		die(" -Failed!");
    	} else {
